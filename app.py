...
# ==============================================================================
# CẤU HÌNH TRANG WEB & LOGO (STYLE SANG TRỌNG)
# ==============================================================================
st.set_page_config(
...
# Nhúng mã CSS tùy biến giao diện cao cấp (Màu vàng nhẹ, trắng ngọc trai, xanh than)
st.markdown("""
    <style>
    ...
    /* Tùy biến thanh sidebar bên trái */
    [data-testid="stSidebar"] {
        background-color: #F4EFE3; /* Nền kem vàng nhẹ */
        border-right: 1px solid #E6D8B8;
    }
    ...
    .result-box {
        ...
        text-align: center;
        margin-top: 15px;
        box-shadow: 0px 4px 15px rgba(197, 160, 89, 0.1);
    }
    .result-amount {
        color: #C5A059;
        font-size: 2rem;
...
# Giao diện Sidebar bên trái
URL_LOGO = "tài chính.png" 
try:
    st.sidebar.image(URL_LOGO, use_container_width=True)
except:
    st.sidebar.markdown("<h2 style='color: #C5A059; text-align:center;'>👑 NHÓM 6</h2>", unsafe_allow_html=True)

st.sidebar.markdown("---")
st.sidebar.write("### 🏢 Hệ thống trực tuyến")
st.sidebar.info("Cổng Đăng Ký Khoản Vay Tự Động\n\nTHUY HOAI - NHÓM 6")
st.sidebar.markdown("---")
st.sidebar.caption("Ứng dụng tự động thẩm định hồ sơ đăng ký vay trực tuyến của Khách hàng cá nhân.")

# ==============================================================================
# TIÊU ĐỀ CHÍNH CỦA ỨNG DỤNG
# ==============================================================================
st.markdown("<div class='main-title'>🏦 ĐĂNG KÝ & THẨM ĐỊNH KHOẢN VAY TRỰC TUYẾN</div>", unsafe_allow_html=True)
st.markdown("<div class='main-subtitle'>Hệ thống tự động chấm điểm tài chính và trả kết quả sơ duyệt sau 3 giây</div>", unsafe_allow_html=True)
...
# ==============================================================================
st.markdown("<h3 class='luxury-subheader'>🪪 1. Thông tin định danh cá nhân</h3>", unsafe_allow_html=True)

# Minh họa bằng hình ảnh sơ đồ quy trình định danh (Identity Verification) hoặc biểu tượng tài chính bảo mật
...
with col_id3:
    dia_chi = st.text_input("Địa chỉ cư trú hiện tại (Số nhà, Đường, Phường/Xã...):", value="123 Đường Lê Lợi, Quận 1, TP. Hồ Chí Minh")

st.markdown("---")

# ==============================================================================
# PHẦN 2 & 3: BỐ CỤC CHIA CỘT GIAO DIỆN HIỆN ĐẠI
# ==============================================================================
col_body1, col_body2 = st.columns(2)

with col_body1:
    st.markdown("<h3 class='luxury-subheader'>📋 2. Nhu cầu vay vốn & Giải pháp</h3>", unsafe_allow_html=True)
    
    # Minh họa hình ảnh về mục đích tài chính hoặc tài sản thế chấp cao cấp
    ...
    if "Không cần tài sản" in loai_vay:
        ...
        GTTSDB = st.number_input("Ước tính giá trị Táì sản bạn định thế chấp (Triệu đồng):", min_value=1.0, value=200.0, step=10.0)
...
with col_body2:
    st.markdown("<h3 class='luxury-subheader'>👤 3. Thông tin tài chính & Lịch sử CIC</h3>", unsafe_allow_html=True)
    
    # Minh họa sơ đồ/hình ảnh đánh giá điểm tín dụng (Credit Score) cá nhân
    ...
    st.markdown("**📌 Lịch sử tín dụng cũ:**")
    tinh_trang_no = st.selectbox(
        "Hiện tại, các khoản vay cũ của bạn có bị trễ hạn thanh toán không?",
        [
...
# ==============================================================================
# PHẦN TÍNH TOÁN DÒNG TIỀN DỰ KIẾN KỲ ĐẦU (REAL-TIME UI BOX)
# ==============================================================================
TG_Thang = TGV * 12
if TG_Thang > 0:
...
st.markdown(f"""
    <div class="result-box">
        <p style="margin: 0; color: #5A6B7C; font-size: 1rem; font-weight: 600;">💡 Ước tính số tiền cần trả hằng tháng (Kỳ đầu tiên)</p>
        <p class="result-amount">{PTMM:.2f} Triệu VND / tháng</p>
        <p style="margin: 0; color: #8A9AAB; font-size: 0.85rem;">
            (Tiền Gốc cố định: {Goc_Hang_Thang:.2f} tr | Tiền Lãi tháng đầu: {PTMM - Goc_Hang_Thang:.2f} tr)
        </p>
    </div>
""", unsafe_allow_html=True)
...
# ==============================================================================
if st.button("📊 GỬI HỒ SƠ VÀ KIỂM TRA KẾT QUẢ", type="primary", use_container_width=True):
    ...
    if not ho_ten.strip():
        ...
        try:
            ...
            # Khởi tạo tabs hiển thị kết quả một cách trực quan chuyên nghiệp
            ...
            with tab1:
                ...
                # Khối logic điều kiện chặn rủi ro hồ sơ
                rejection_reasons = []
            ...
            with tab2:
                ...
                st.write(f"- 💵 **Tiền trả định kỳ cho khoản vay mới này:** `{PTMM:.2f}` Triệu đồng/tháng")
...
# Chân trang (Footer)
st.markdown("<br><hr><p style='text-align: center; color: #8A9AAB; font-size: 0.85rem;'>© 2026 Hệ thống tài chính cá nhân cao cấp. Bảo lưu mọi quyền.</p>", unsafe_allow_html=True)
